...
class SubagentManager:
    ...
    def _build_tool_registry(self) -> ToolRegistry:
        """Build the tool registry for the subagent."""
        tools = ToolRegistry()
    ...
    async def _handle_tool_calls(
        ...
        """Handle tool calls from the subagent."""
    ...
    async def _run_subagent_loop(
        self, messages: List[Dict[str, Any]], tools: ToolRegistry, task_id: str
    ) -> str:
        """Run the main subagent loop."""
        for _ in range(MAX_SUBAGENT_ITERATIONS):
            ...
            if response.has_tool_calls:
                await self._handle_tool_calls(response, messages, tools, task_id)
            else:
    ...
    async def _run_subagent(
        ...
        try:
            ...
            final_result = await self._run_subagent_loop(messages, tools, task_id)
